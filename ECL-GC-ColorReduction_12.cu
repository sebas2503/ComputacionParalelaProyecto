...
static __global__ __launch_bounds__(ThreadsPerBlock, 2048 / ThreadsPerBlock)
void reduceInit(const int nodes, const int* const __restrict__ color, int* const __restrict__ hic)
{
  // find highest color
...
static __global__ __launch_bounds__(ThreadsPerBlock, 2048 / ThreadsPerBlock)
void reduceInit2(const int nodes, const int* const __restrict__ nidx, const int* const __restrict__ nlist, const int* const __restrict__ color, int* const __restrict__ wl, int* const __restrict__ wlsize, const int* const __restrict__ hic, bool* const __restrict__ m)
{
  ...
  //initialize matrix m
  for (int i = from; i < (h * h); i += incr) m[i] = false;
}


static __global__ __launch_bounds__(ThreadsPerBlock, 2048 / ThreadsPerBlock)
void reduceFind(const int* const __restrict__ nidx, const int* const __restrict__ nlist, const int* const __restrict__ color, const int* const __restrict__ wl, const int* const __restrict__ wlsize, const int* const __restrict__ hic, bool* const __restrict__ m)
{
  const int from = threadIdx.x + blockIdx.x * ThreadsPerBlock;
  const int incr = gridDim.x * ThreadsPerBlock;
  const int h = *hic;
...
static __global__ __launch_bounds__(ThreadsPerBlock, 2048 / ThreadsPerBlock)
void reduceFind2(const int* const hic, const bool* const m)
{
  const int from = threadIdx.x + blockIdx.x * ThreadsPerBlock;
  const int incr = gridDim.x * ThreadsPerBlock;
  const int h = *hic;
...
static __global__ __launch_bounds__(ThreadsPerBlock, 2048 / ThreadsPerBlock)
void reduceRecolor(const int nodes, int* const __restrict__ color, const int* const __restrict__ wl, const int* const __restrict__ wlsize, const int* const __restrict__ hic)
{
...
static __global__ __launch_bounds__(ThreadsPerBlock, 2048 / ThreadsPerBlock)
void reduce2Init(const int nodes, const int* const __restrict__ color, int* const __restrict__ hic, int* const __restrict__ nstate)
{
  // find highest color
  const int from = threadIdx.x + blockIdx.x * ThreadsPerBlock;
  const int incr = gridDim.x * ThreadsPerBlock;
...
static __global__ __launch_bounds__(ThreadsPerBlock, 2048 / ThreadsPerBlock)
void reduce2Init2(const int nodes, const int* const __restrict__ color, int* const __restrict__ wl, int* const __restrict__ wlsize, const int* const __restrict__ hic, int* const __restrict__ nstate)
{
  const int from = threadIdx.x + blockIdx.x * ThreadsPerBlock;
  const int incr = gridDim.x * ThreadsPerBlock;
...
static __global__ __launch_bounds__(ThreadsPerBlock, 2048 / ThreadsPerBlock)
void redue2ComputeUnion(const int nodes, const int* const __restrict__ nidx, const int* const __restrict__ nlist, int* const __restrict__ nstate, const int* const __restrict__ color, const int* const __restrict__ hic)
{
  const int from = threadIdx.x + blockIdx.x * ThreadsPerBlock;
  const int incr = gridDim.x * ThreadsPerBlock;
...
static __global__ __launch_bounds__(ThreadsPerBlock, 2048 / ThreadsPerBlock)
void reduce2Conflict1(const int nodes, const int* const __restrict__ nidx, const int* const __restrict__ nlist, int* const __restrict__ nstate, const int* const __restrict__ color, const int* const __restrict__ hic, int* const __restrict__ hicSet, int* const __restrict__ posscolor)
{
  const int from = threadIdx.x + blockIdx.x * ThreadsPerBlock;
  const int incr = gridDim.x * ThreadsPerBlock;
  ...
  for (int v = from; v < nodes; v += incr) {
    hicSet[v] = -1;
    for (int j = nidx[v]; j < nidx[v + 1]; j ++) {  // their neighbors
  ...
  for (int i = from; i < nodes * 32; i += incr) posscolor[i] = (1 << (i % 32)) | mask;
}


static __global__ __launch_bounds__(ThreadsPerBlock, 2048 / ThreadsPerBlock)
void reduce2MarkColors(const int* const __restrict__ nidx, const int* const __restrict__ nlist, int* const __restrict__ nstate, const int* const __restrict__ color, int* const __restrict__ posscolor, const int* const __restrict__ wl, const int* const __restrict__ wlsize)
{
  const int from = threadIdx.x + blockIdx.x * ThreadsPerBlock;
  const int incr = gridDim.x * ThreadsPerBlock;
  // for all neighbors with color c of all hic vertices, record what colors their neighbors are using (under 32 only)
  for (int i = from; i < *wlsize; i += incr) {  // all hic vertices
    const int v = wl[i];
    ...
    for (int j = nidx[v]; j < nidx[v + 1]; j++) {  // their neighbors
      ...
      if (ncol < 32) {
        ...
        for (int k = nidx[nlj]; k < nidx[nlj + 1]; k++) {  // the neighbors' neighbors
...
static __global__ __launch_bounds__(ThreadsPerBlock, 2048 / ThreadsPerBlock)
void reduce2Conflict2(const int nodes, const int* const __restrict__ nidx, const int* const __restrict__ nlist, const int* const __restrict__ color, int* const __restrict__ posscolor, const int* const __restrict__ hicSet)
{
  const int from = threadIdx.x + blockIdx.x * ThreadsPerBlock;
  const int incr = gridDim.x * ThreadsPerBlock;
...
static __global__ __launch_bounds__(ThreadsPerBlock, 2048 / ThreadsPerBlock)
void reduce2Recolor(const int nodes, const int* const __restrict__ nidx, const int* const __restrict__ nlist, int* const __restrict__ color, int* const __restrict__ posscolor, const int* const __restrict__ wl, const  int* const __restrict__ wlsize, int* const __restrict__ nstate, const int* const __restrict__ hic)
{
  const int from = threadIdx.x + blockIdx.x * ThreadsPerBlock;
  const int incr = gridDim.x * ThreadsPerBlock;
...
int main(int argc, char* argv[])
{
  ...
  const int vals_1 = 16;
  int c_1[vals_1];
  for (int i = 0; i < vals_1; i++) c_1[i] = 0;
  int cols1 = -1;
  for (int v = 0; v < g.nodes; v++) {
    cols1 = std::max(cols1, color[v]);
    if (color[v] < vals_1) c_1[color[v]]++;
  }
  cols1++;
  ...
  if (avg_deg > 10) {
    ...
    bool* m_gpu;
    int hic = INT_MIN;
    if (cudaSuccess != cudaMalloc((void **)&wl_gpu, g.nodes * sizeof(int)))fprintf(stderr, "ERROR: could not allocate wl\n");
    if (cudaSuccess != cudaMalloc((void **)&wlsize_gpu, sizeof(int))) fprintf(stderr, "ERROR: could not allocate wlsize\n");
    if (cudaSuccess != cudaMalloc((void **)&hic_gpu, sizeof(int))) fprintf(stderr, "ERROR: could not allocate hic_gpu\n");
    if (cudaSuccess != cudaMemcpy(hic_gpu, &hic, sizeof(int), cudaMemcpyHostToDevice)) fprintf(stderr, "ERROR: copying hic to device failed\n");
    timer.start();
    reduceInit<<<blocks, ThreadsPerBlock>>>(g.nodes, color_d, hic_gpu);
    if (cudaSuccess != cudaMemcpy(&hic, hic_gpu, sizeof(int), cudaMemcpyDeviceToHost)) fprintf(stderr, "ERROR: copying of hic from device failed\n");
    if (cudaSuccess != cudaMalloc((void **)&m_gpu, (hic * hic) * sizeof(bool))) fprintf(stderr, "ERROR: could not allocate m\n");
    ...
    cudaFree(wl_gpu); cudaFree(wlsize_gpu); cudaFree(hic_gpu); cudaFree(m_gpu);
  } else {
    // use second heuristic
    int h_hic = INT_MIN;
    int h_wlsize = 0;
    int *hicSet1, *hic, *posscolor, *nstate, *wl, *wlsize2;
    if (cudaSuccess != cudaMalloc((void **)&hicSet1, g.nodes * sizeof(int))) {printf("ERROR: could not allocate hicSet\n\n");  exit(-1);}
    if (cudaSuccess != cudaMalloc((void **)&hic, sizeof(int))) {printf("ERROR: could not allocate hicSet\n\n");  exit(-1);}
    if (cudaSuccess != cudaMalloc((void **)&nstate, g.nodes * sizeof(int))) {printf("ERROR: could not allocate nstate\n\n");  exit(-1);}
    if (cudaSuccess != cudaMalloc((void **)&wl, g.nodes * sizeof(int))) {printf("ERROR: could not allocate wl\n\n");  exit(-1);}
    if (cudaSuccess != cudaMalloc((void **)&wlsize2, sizeof(int))) {printf("ERROR: could not allocate wlsize2,\n\n");  exit(-1);}
    if (cudaSuccess != cudaMemcpy(hic, &h_hic, sizeof(int), cudaMemcpyHostToDevice)) fprintf(stderr, "ERROR: copying hic to device failed\n");
    if (cudaSuccess != cudaMemcpy(wlsize2, &h_wlsize, sizeof(int), cudaMemcpyHostToDevice)) fprintf(stderr, "ERROR: copying wlsize to device failed\n");
    if (cudaSuccess != cudaMalloc((void **)&posscolor, (g.nodes * 32 ) * sizeof(int))) {printf("ERROR: could not allocate posscolor\n\n");  exit(-1);}
    timer.start();
    ...
    cudaFree(hicSet1); cudaFree(posscolor); cudaFree(nstate); cudaFree(wlsize2), cudaFree(hic), cudaFree(wl);
  }
  if (cudaSuccess != cudaMemcpy(color, color_d, g.nodes * sizeof(int), cudaMemcpyDeviceToHost)) {printf("ERROR: copying color from device failed\n\n");  exit(-1);}
